...
type serverApp struct {
	*ServerCommand
	...
	refreshCache *authRefreshCache // stored only to close it properly on shutdown
}
...
func (s *ServerCommand) newServerApp() (*serverApp, error) {
	...
	authenticator, err := s.makeAuthenticator(dataService, avatarStore, adminStore, refreshCache)
	if err != nil {
...
func (a *serverApp) run(ctx context.Context) error {
	...
	if e := a.refreshCache.Close(); e != nil {
		log.Printf("[WARN] failed to close auth refreshCache, %s", e)
	}
...
func (s *ServerCommand) makeAuthenticator(ds *service.DataStore, avas avatar.Store, admns admin.Store, refreshCache *authRefreshCache) (*auth.Service, error) {
